...
function showthumbs($aantal = '10', $hide = '') {
    ...
    $startat    = ( ( $currentpage * $aantal ) - $aantal ) + 1;
    $endat      = ( $currentpage * $aantal );

    echo '<section id="andere"><h2>Wat anderen maakten:</h2>';
    echo '<div id="andere-container">';
    ...
    if ( $previouspage > 0) {
        echo '<li><a href="' . $desturl . '/' . TEGELIZR_PAGEURL . '/' . $previouspage . '/" class="previouspage thumbnav" title="vorige ' .$aantal . '"><span>vorige ' .$aantal . '</span><span class="pijl">➮</span></a></li>';
    }
...
//                        echo 'ja (' . $counter . ' - ' . $startat .'/'. $endat . ')';
    ...
    if ( $nextpage > 1) {
        echo '<li><a href="' . $desturl . '/' . TEGELIZR_PAGEURL . '/' . $nextpage . '/" class="nextpage thumbnav" title="volgende ' .$aantal . '"><span>volgende ' .$aantal . '</span><span class="pijl">➭</span></a></li>';
    }
    echo '</ul>';
    echo '</div>';
    echo '</section>';
...
function wbvb_d2e_socialbuttons($thelink = 'thelink', $thetitle = 'thetitle', $summary = 'summmary') {
    ...
    $sitetitle  = rawurlencode($thetitle);
    ...
    if ( $thelink ) {
        return '<ul class="social-media">
            <li><a href="https://twitter.com/share?url=' . rawurlencode($thelink) . '&amp;via=paulvanbuuren&amp;text=' . rawurlencode($thetitle) . '" class="twitter" data-url="' . $thelink . '" data-text="' . $thetitle . '" data-via="@paulvanbuuren"' . $popup . '><span>Tweet</span></a></li><li><a class="facebook" href="https://www.facebook.com/sharer/sharer.php?u=' . rawurlencode($thelink) . '&amp;t=' . rawurlencode($thetitle) . '"' . $popup . '><span>Facebook</span></a></li><li><a class="linkedin" href="http://www.linkedin.com/shareArticle?mini=true&amp;url=' . rawurlencode($thelink) . '&amp;title=' . rawurlencode($thetitle) . '&amp;summary=' . rawurlencode($summary) . '&amp;source=' . $sitetitle . '"' . $popup . '><span>LinkedIn</span></a></li></ul>';    
    }
}
...
// ===================================================================================================================
function htmlheader() {
	if ( 'appelflap.local:5757' == $_SERVER["HTTP_HOST"] || 'tegelizr:8185' == $_SERVER["HTTP_HOST"] ) { 
        return '<link href="css/stylesheet.css" rel="stylesheet" type="text/css"></head><body class="nojs">';
    }
    else {
        return '</head><body class="nojs">';
    }
}
...
function includejs() {
	if ( 'appelflap.local:5757' == $_SERVER["HTTP_HOST"] || 'tegelizr:8185' == $_SERVER["HTTP_HOST"] ) { 
    	$jquery = '<scri' . 'pt src="/min/jquery-latest.min.js"></scri' . 'pt>';
    } 
    else {
    	$jquery = '<scri' . 'pt src="http://code.jquery.com/jquery-latest.min.js"></scri' . 'pt>';
    }
    return $jquery . '
    <scri' . 'pt src="/min/jquery.mThumbnailScroller.js"></scri' . 'pt>
    <scr' . 'ipt>	
		(function($){
