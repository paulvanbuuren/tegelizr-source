...
//        $images = glob($outpath_thumbs . "*.jpg");
        ...
        if ( $pagenumber > 1 ) {
            $linkerknop = '<div class="linkerknop"><button type="submit" class="get_previous" name="pagenumber" value="' . ( $pagenumber - 1 ). '"><span class="label">' . TEGELIZR_VORIGE . '</span>' . HTML_PIJL_VORIGE . '</button></div>';
...
//        $alletegeltje = '<p><a href="' . TEGELIZR_PROTOCOL . $_SERVER["HTTP_HOST"] . '/' . TEGELIZR_ALLES . '/">alle tegeltjes</a></p>';
        ...
        if ( ( $currentpage < $maxnumberofpages ) ) {
            ...
            $rechterknop = '<div class="rechterknop"><button type="submit" class="get_previous" name="pagenumber" value="' . ( $pagenumber + 1 ). '"><span class="label">' . TEGELIZR_VOLGENDE . '</span>' . HTML_PIJL_VOLGENDE . '</button></div>';
...
function htmlheader($bodyclass = '') {
    ...
    if ( PVB_DEBUG ) {
        ...
        if ( SHOW_STRUCTURE ) {
            ...
            $style .= 'font-family: Subway,Helvetica,Arial,serif;';
            ...
            $style .= 'background: rgba(250,250,250,.4);';
            $style .= 'border: 1px solid rgba(124,124,124,.7);';
            $style .= '}';
            $style .= 'a.placeholder {';
...
//    $extra .= '<h1 style="border: 10px solid red; background: white; color: black; z-index: 999;">JAHAHAHA</h1>';    
...
function getSearchResultItem($result, $showImage = true) {
    ...
    $return =  '<section>';
    if ( $showImage ) {
        $return .= '<div><a href="/'  . TEGELIZR_SELECTOR . '/' . $hashname . '" title="' . filtertext($result['txt_tegeltekst'], true) . ' - ' . $result[TEGELIZR_VIEWS] . ' keer bekeken" class="tegeltje"><img src="/' . TEGELIZR_THUMBS . '/' . $thumb . '" height="' . TEGELIZR_THUMB_WIDTH . '" width="' . TEGELIZR_THUMB_WIDTH . '" alt="' . filtertext($result['txt_tegeltekst'], true ) . '" /></a></div>';        
    }
...
function returnheader($pagetitle = '', $atitle = '', $withlinks = true, $tag = 'h1') {
    ...
    $return .= returnlogo("Redactie", "Maak zelf ook een tegeltje") . '<span>' . $pagetitle . '</span>';
    if ( $withlinks ) {
        $return .=  '</a>';        
    }
    $return .= '</' . $tag . '>' . return_nav_menu() . '</header>';
    return $return;
...
function return_nav_menu() {
    return '<div id="menu" class="menu js-menu" role="navigation"></div>';
//    return '<nav id="menu" class="menu js-menu" role="navigation"><ul><li><a href="' . TEGELIZR_PROTOCOL . $_SERVER["HTTP_HOST"] . '/' . TEGELIZR_REDACTIE . '/">redactie</a></li><li><a href="' . TEGELIZR_PROTOCOL . $_SERVER["HTTP_HOST"] . '/' . TEGELIZR_ALLES . '/">alle tegeltjes</a></li><li><a href="http://wbvb.nl/tegeltjes-maken-is-een-keuze/">waarom tegeltjes</a></li></ul></nav>';
...
// =========================================================================================================
function menu_naar_footer () {

    var newParent = document.getElementById('documentfooter');
    var oldParent = document.getElementById('menu');
...
// =========================================================================================================
function menu_naar_header () {

    var newParent = document.getElementById('menu');
    var oldParent = document.getElementById('documentfooter');
...
// ===================================================================================================================
// function voor navigatie door alle tegeltjes
// ===================================================================================================================    
...
function writecontrolform() {
    ...
    if ( $pagenumber > 1 ) {
        $returnstring .= '<button type="submit" class="get_previous" name="pagenumber" value="' . ( $pagenumber - 1 ). '">' . TEGELIZR_VORIGE . '</button>';
    }
    ...
    $returnstring .= '<button type="submit" class="get_next" name="pagenumber" value="' . ( $pagenumber + 1 ). '">' . TEGELIZR_VOLGENDE . '</button>';
    $returnstring .= '</div>';
    $returnstring .= '</fieldset>';
    $returnstring .= '</form>';
